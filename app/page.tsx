...
import { 
  Timer, Activity, ChevronRight, TrendingUp, TrendingDown,
...
// Vivid, distinct palette — no two look alike on dark background
const CHART_PALETTE = [
  '#FF8000', '#00D2BE', '#E80020', '#a855f7', '#FFD700',
  '#00FF88', '#FF69B4', '#38bdf8', '#f97316', '#84cc16'
]

// 2026 F1 Race Calendar — update dates if schedule changes
const RACE_CALENDAR: { name: string; date: string }[] = [
  { name: 'Australian GP',   date: '2026-03-15T05:00:00Z' },
  { name: 'Chinese GP',      date: '2026-03-22T07:00:00Z' },
  { name: 'Japanese GP',     date: '2026-04-06T05:00:00Z' },
  { name: 'Bahrain GP',      date: '2026-04-19T15:00:00Z' },
  { name: 'Saudi Arabian GP',date: '2026-04-26T17:00:00Z' },
  { name: 'Miami GP',        date: '2026-05-10T19:00:00Z' },
  { name: 'Monaco GP',       date: '2026-05-24T13:00:00Z' },
  { name: 'Spanish GP',      date: '2026-06-07T13:00:00Z' },
  { name: 'Canadian GP',     date: '2026-06-14T18:00:00Z' },
  { name: 'Austrian GP',     date: '2026-06-28T13:00:00Z' },
  { name: 'British GP',      date: '2026-07-05T14:00:00Z' },
  { name: 'Belgian GP',      date: '2026-07-26T13:00:00Z' },
  { name: 'Hungarian GP',    date: '2026-08-02T13:00:00Z' },
  { name: 'Dutch GP',        date: '2026-08-30T13:00:00Z' },
  { name: 'Italian GP',      date: '2026-09-06T13:00:00Z' },
  { name: 'Madrid GP',       date: '2026-09-13T13:00:00Z' },
  { name: 'Azerbaijan GP',   date: '2026-09-20T11:00:00Z' },
  { name: 'Singapore GP',    date: '2026-10-04T08:00:00Z' },
  { name: 'US GP',           date: '2026-10-18T19:00:00Z' },
  { name: 'Mexican GP',      date: '2026-10-25T19:00:00Z' },
  { name: 'Brazilian GP',    date: '2026-11-08T17:00:00Z' },
  { name: 'Las Vegas GP',    date: '2026-11-21T06:00:00Z' },
  { name: 'Qatar GP',        date: '2026-11-29T13:00:00Z' },
  { name: 'Abu Dhabi GP',    date: '2026-12-06T13:00:00Z' },
]

function getNextRace() {
  const now = Date.now()
  return RACE_CALENDAR.find(r => new Date(r.date).getTime() > now) 
    ?? RACE_CALENDAR[RACE_CALENDAR.length - 1]
}
...
export default function Home() {
  ...
  // --- Dynamic countdown to next race ---
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setNextRace(current)
      const diff = new Date(current.date).getTime() - Date.now()
      if (diff <= 0) {
    ...
    Papa.parse(SHEET_URL + '&cachebust=' + Date.now(), {
      ...
      complete: (results) => {
        ...
        if (rows.length > 0) {
          ...
          const CHANGE_KEY  = find(firstRow, 'last change', 'last change\n', 'elo change', 'change', 'delta') || ''
          ...
          // Default: top 5 by ELO rating
          if (selectedChartDrivers.length === 0) {
            const top5 = [...parsed]
              .sort((a, b) => b.elo - a.elo)
              .slice(0, 5)
              .map(d => d.driver)
            setSelectedChartDrivers(top5)
          }

          // --- Dynamic completed race detection ---
          // Sheet structure per race: XX RACE_1 (ELO) | _N (delta) | _N+1 (position) | _N+2 (?)
          // A race has happened if ANY driver has a finishing position between 1–22
          ...
          const raceColumns = allKeys
            ...
            .filter(eloKey => {
              ...
              const posKey = allKeys[eloIdx + 2] // position sub-column
              if (!posKey) return false
              return rows.some(row => {
                const pos = parseInt(row[posKey])
                return pos >= 1 && pos <= 22
              })
            })
      ...
      <section className="relative w-full border-b border-zinc-800 bg-[#0a0a0c] transition-colors hover:bg-[#0c0c0e]">
        <Link href="/editorial" className="group block container mx-auto grid grid-cols-1 lg:grid-cols-12 cursor-pointer">
          ...
          <div className="lg:col-span-4 bg-[#08080a] p-8 flex flex-col justify-between group-hover:bg-[#0a0a0c] transition-colors relative overflow-hidden">
                ...
                {[
                  ...
                  <div key={stat.label}>
                    ...
                    <div className="h-1 w-full bg-zinc-900 rounded-full overflow-hidden">
                      <div className="h-full bg-orange-600 transition-all duration-1000 shadow-[0_0_10px_rgba(234,88,12,0.5)]" style={{ width: `${stat.percent}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-orange-600/5 rounded-full blur-3xl pointer-events-none" />
          </div>
        </Link>
      </section>
      ...
      <section className="container mx-auto px-6 mt-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-6">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter">Season Trajectory</h2>
          </div>
          ...
          <div className="bg-[#0a0a0c] border border-zinc-800 rounded-xl px-6 py-4 shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center gap-6">
            <div className="hidden sm:block text-right pr-6 border-r border-zinc-800">
              ...
              <p className="font-bold text-sm uppercase text-orange-500 italic">{nextRace.name}</p>
            </div>
            <div className="flex items-center gap-4 text-center tabular-nums font-mono font-black">
              {([
                ...
                <div key={idx} className="flex flex-col">
                  <span className="text-2xl md:text-3xl text-zinc-100 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{t.val}</span>
                  <span className="text-[9px] text-zinc-600 uppercase tracking-widest">{t.label}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div style={{ width: '100%', height: '100%', minHeight: 400 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                    ...
                    {selectedChartDrivers.map((driverId, index) => {
        ...
        <div className="mt-6 flex flex-wrap gap-2 items-center">
          <span className="text-[10px] font-black uppercase italic tracking-widest text-zinc-600 mr-2">Target Focus:</span>
          {drivers.slice(0, 15).map((d) => {
            const isSelected = selectedChartDrivers.includes(d.driver)
            const paletteColor = CHART_PALETTE[selectedChartDrivers.indexOf(d.driver) % CHART_PALETTE.length]
            const chipColor = isSelected ? paletteColor : (TEAM_COLORS[d.team.toLowerCase()] || '#8a8a94')
            return (
              <button
                key={d.driver}
                ...
                style={{
                  borderColor: isSelected ? paletteColor : undefined,
                  boxShadow: isSelected ? `0 0 10px ${paletteColor}40` : undefined,
                }}
              >
