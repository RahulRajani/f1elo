...
import {
  ...
  BarChart3, Zap, Target, Share2, Download, X
} from 'lucide-react'
import {
  ...
  useEffect(() => {
    Papa.parse(SHEET_URL + '&cachebust=' + Date.now(), {
      ...
      complete: (results) => {
        ...
        const CHANGE_KEY = find(fr, 'last change', 'elo change', 'change') || ''
        ...
        const found = parsed.find(d => d.driver.toLowerCase() === driverName.toLowerCase())
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-400 uppercase tracking-widest text-sm font-bold">Loading driver profile...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="bg-gradient-to-br from-zinc-900/80 to-black border border-zinc-700/50 rounded-xl shadow-xl overflow-hidden mb-12">
          ...
          <div className="relative w-full h-[400px] py-6 px-6">
            {driver.history.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-zinc-600 text-sm uppercase">No race data available</div>
            )}
          </div>
        </div>
